...
contract Broker is ERC721Holder {
    ...
    // list of all FixedPriceTokensForSale
    ...
    // list of all AuctionTokensForSale
    ...
    // set brokerage amount (onlyowner)
    function setBrokerage(uint16 _brokerage) public onlyOwner {
        brokerage = _brokerage;
    }
    // A buyer will bid on tokenforsale
    function bid(
        uint256 tokenID,
    ...
    function collect(
        ...
        if (
            ...
            erc20TokensArray.array[ercIndex] &&
            exists == false
        ) {
    ...
    function buy(
        ...
        if (
            ...
            require(
                auctions[_mintableToken][tokenID].erc20Token == _erc20Token,
                "the erc20Token not match "
            );
